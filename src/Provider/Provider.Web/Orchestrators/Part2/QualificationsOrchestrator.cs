...
public class QualificationsOrchestrator(
    ...
    : VacancyValidatingOrchestrator<QualificationEditModel>(logger)
{
    public async Task<QualificationsViewModel> GetQualificationsViewModelAsync(VacancyRouteModel vrm)
    {
        var vacancy = await utility.GetAuthorisedVacancyForEditAsync(vrm, RouteNames.Qualifications_Get);

        var qualifications = vacancy.Qualifications ?? [];
    ...
    {
        var vacancy = await utility.GetAuthorisedVacancyForEditAsync(m, RouteNames.Qualifications_Get);
    ...
    {
        var vacancyTask = utility.GetAuthorisedVacancyForEditAsync(vrm, RouteNames.Qualification_Add_Get);
        var allQualificationsTask = referenceDataClient.GetCandidateQualificationsAsync();
    ...
    {
        var vacancyTask = utility.GetAuthorisedVacancyForEditAsync(vrm, RouteNames.Qualification_Edit_Get);
        var allQualificationsTask = referenceDataClient.GetCandidateQualificationsAsync();
    ...
    {
        var vacancy = await utility.GetAuthorisedVacancyForEditAsync(vrm, RouteNames.Qualification_Add_Post);
    ...
    {
        var vacancy = await utility.GetAuthorisedVacancyForEditAsync(vrm, RouteNames.Qualification_Edit_Post);
    ...
    private QualificationViewModel GetQualificationViewModel(Vacancy vacancy, IList<string> allQualifications, VacancyRouteModel vrm)
    {
        ...
        vm.BackRoute = utility.IsTaskListCompleted(vacancy) ? RouteNames.ProviderCheckYourAnswersGet : backRoute;
    ...
    private async Task<OrchestratorResponse> UpdateVacancyWithQualificationAsync(Vacancy vacancy, Qualification currentQualification, Qualification qualification, QualificationEditModel m, VacancyUser user)
    {
        ...
        var qualificationTypes = await referenceDataClient.GetCandidateQualificationsAsync();
        ...
        return await ValidateAndExecute(vacancy,
            v =>
            {
                var result = vacancyClient.ValidateQualification(qualification);
                SyncErrorsAndModel(result.Errors, m);
                return result;
            },
            v => vacancyClient.UpdateDraftVacancyAsync(v, user));
    }
        ...
        AddQualificationsEditModel m, VacancyUser user)
    {
        ...
        return await ValidateAndExecute(
            vacancy,
            v => vacancyClient.Validate(v, VacancyRuleSet.None),
            v => vacancyClient.UpdateDraftVacancyAsync(v, user));
    }
