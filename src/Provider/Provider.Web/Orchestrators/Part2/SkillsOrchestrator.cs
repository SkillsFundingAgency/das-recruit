...
public class SkillsOrchestrator(
    ...
    : VacancyValidatingOrchestrator<SkillsEditModel>(logger)
{
    private const VacancyRuleSet ValidationRules = VacancyRuleSet.Skills;
    private readonly SkillsOrchestratorHelper _skillsHelper = new(() => referenceDataClient.GetCandidateSkillsAsync().Result);

    public async Task<SkillsViewModel> GetSkillsViewModelAsync(VacancyRouteModel vrm, string[] draftSkills = null)
    {
        var vacancy = await utility.GetAuthorisedVacancyForEditAsync(vrm, RouteNames.Skills_Get);
    ...
    {
        var vacancy = await utility.GetAuthorisedVacancyForEditAsync(vrm, RouteNames.Skills_Post);
        ...
        return await ValidateAndExecute(vacancy,
            v =>
            {
                var result = vacancyClient.Validate(v, ValidationRules);
                SyncErrorsAndModel(result.Errors, m, vacancy);
                return result;
            },
            v => validateOnly ? Task.CompletedTask : vacancyClient.UpdateDraftVacancyAsync(v, user));
    }
