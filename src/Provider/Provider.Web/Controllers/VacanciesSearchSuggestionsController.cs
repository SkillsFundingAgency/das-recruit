...
namespace Esfa.Recruit.Provider.Web.Controllers
{
    ...
    public class VacanciesSearchSuggestionsController : Controller
    {
        ...
        public async Task<JsonResult> GetSearchSuggestions([FromQuery]string searchTerm, VacancyRouteModel model)
        {
            var data = await _orchestrator.GetSearchSuggestionsAsync(searchTerm?.Trim(), model.Ukprn);
            return Json(data);
        }
    }
}