...
const buttons = {
    'bullet-list' : {
        ...
        'aria-keyshortcuts': 'Control+Shift+8 Meta+Shift+8',
...
function createToolbarBtn(name, ariaLabelName) {
    ...
    // create the svg container for the icon
    ...
    const ariaLabelText = ariaLabelName ? `${buttons[name]['aria-label']} for ${ariaLabelName}` : buttons[name]['aria-label']
    btn.setAttribute('aria-label', ariaLabelText)
    
    if (buttons[name].hasOwnProperty('aria-keyshortcuts')) {
        btn.setAttribute('aria-keyshortcuts', buttons[name]['aria-keyshortcuts'])
    }
